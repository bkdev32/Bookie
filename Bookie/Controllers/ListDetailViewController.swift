...
class ListDetailViewController: UITableViewController {
    ...
    func loadList(_ list: String) {
        session.db.collection(B.Fire.collection).whereField(B.Fire.status, isEqualTo: list)
                    ...
                    if query?.isEmpty != true && query != nil {
                        for book in query!.documents {
                            if let id = book.get(B.Fire.id) as? String,
                               ...
                               let status = book.get(B.Fire.status) as? String,
                            ...
                            {
                                let newBook = Book(addedBy: addedBy, status: status, id: id, title: title, authors: authors, date: date, desc: desc, categories: categories, pageCount: pageCount, rating: rating, link: link, image: image, smallImage: smallImage)
...
extension ListDetailViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let index = tableView.indexPathForSelectedRow {
            let book = Book(addedBy: books[index.row].addedBy, status: books[index.row].status, id: books[index.row].id, title: books[index.row].title, authors: books[index.row].authors, date: books[index.row].date, desc: books[index.row].desc, categories: books[index.row].categories, pageCount: books[index.row].pageCount, rating: books[index.row].rating, link: books[index.row].link, image: books[index.row].image, smallImage: books[index.row].smallImage)
